...
rs_fcs = np.loadtxt("../eegvggcorrs_cal/avg_corrs/fcs/trained/theta_power_fp.txt")[:, 113:]
...
ps_convs = np.loadtxt("../eegvggcorrs_cal/avg_stats_results/convs/trained/theta_power_fp.txt")[113:]
ps_fcs = np.loadtxt("../eegvggcorrs_cal/avg_stats_results/fcs/trained/theta_power_fp.txt")[113:]
ps_convsfcs = np.loadtxt("../eegvggcorrs_cal/avg_stats_results/convs-fcs/trained/theta_power_fp.txt")[113:]

t = 0
ps_convs[737] = 1
while t < 738:
    if ps_convs[t] < 0.05 and avg_rs_convs[t] > 0:
        index = 0
        while ps_convs[t+index] < 0.05 and avg_rs_convs[t+index] > 0:
            index = index + 1
        if index > 25:
            t = t + index
        if index <= 25:
...
ps_fcs[737] = 1
while t < 738:
    if ps_fcs[t] < 0.05 and avg_rs_fcs[t] > 0:
        index = 0
        print(t)
        while ps_fcs[t+index] < 0.05 and avg_rs_fcs[t+index] > 0 and t+index < 738:
            index = index + 1
        if index > 25:
            t = t + index
        if index <= 25:
...
ps_convsfcs[737] = 1
while t < 738:
    if ps_convsfcs[t] < 0.05 and avg_rs_convs[t] > 0 and avg_rs_fcs[t] > 0:
        index = 0
        while ps_convsfcs[t+index] < 0.05 and avg_rs_convs[t+index] > 0 and avg_rs_fcs[t+index] > 0:
            index = index + 1
        if index > 25:
            t = t + index
        if index <= 25:
...
for t in range(738):
    ...
    if ps_convsfcs[t] < 0.05 and avg_rs_convs[t] > 0:
